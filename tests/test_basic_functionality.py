...
def starlette_app():
    ...
    routes = [
        Route(
            ...
            cached_handler,
            methods=["GET"],
            dependencies=[Depends(create_cache_config)],
        ),
...
class TestMiddlewareRouteAnalysis:
    ...
    def test_route_analysis_fastapi(
        self, fastapi_app: FastAPI, storage: InMemoryStorage
    ) -> None:
    ...
    def test_cache_config_extraction(
        self, fastapi_app: FastAPI, storage: InMemoryStorage
    ) -> None:
...
class TestMiddlewareCaching:
    ...
    @pytest.mark.asyncio
    async def test_cache_miss_and_storage(
        self, fastapi_app: FastAPI, storage: InMemoryStorage
    ) -> None:
        ...
        request = Request(scope, AsyncMock())
        metadata = {"cached_at": datetime.utcnow().isoformat(), "ttl": 60}
